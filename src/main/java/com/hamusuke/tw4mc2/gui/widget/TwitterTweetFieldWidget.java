...
public class TwitterTweetFieldWidget extends Widget implements IRenderable, IGuiEventListener {
	private static final Logger LOGGER = LogManager.getLogger();
	private final FontRenderer textRenderer;
	private String text;
	private TweetText tweetText;
	private List<IReorderingProcessor> wrappedTextList = Lists.newArrayList();
	...
	public TwitterTweetFieldWidget(FontRenderer textRenderer, int x, int y, int width, int height, ITextComponent message) {
		this(textRenderer, x, y, width, height, null, message);
	}

	public TwitterTweetFieldWidget(FontRenderer textRenderer, int x, int y, int width, int height, @Nullable TextFieldWidget copyFrom, ITextComponent message) {
	...
	private static List<IReorderingProcessor> wrapLines(String string) {
		return Arrays.stream(string.split("\n")).map(TweetText::new).map(TweetText::getVisualOrderText).collect(Collectors.toList());
	}
	...
	public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		if (this.isVisible()) {
			...
			if (!bl3 && this.suggestion != null) {
				this.textRenderer.drawShadow(matrices, this.suggestion, (float) (o - 1), (float) m + c, -8355712);
			}
			...
			if (bl2) {
				if (bl3) {
					...
					this.textRenderer.drawShadow(matrices, "_", (float) o, (float) m + c, i);
				}
			}
	...
	private int getWidth(String text) {
		return ((TextRendererInvoker) this.textRenderer).getWidthWithEmoji(new TweetText(text).getVisualOrderText());
	}
