...
public class MaskableTextFieldWidget extends Widget implements IRenderable, IGuiEventListener {
    ...
    public MaskableTextFieldWidget(FontRenderer textRenderer, int x, int y, int width, int height, ITextComponent message, char mask, int maxLength) {
        this(textRenderer, x, y, width, height, null, message, mask, maxLength);
    }

    public MaskableTextFieldWidget(FontRenderer textRenderer, int x, int y, int width, int height, @Nullable MaskableTextFieldWidget copyFrom, ITextComponent message, char mask, int maxLength) {
    ...
    public void write(String text) {
        String string = "";
        String string2 = SharedConstants.filterText(text);
        int i = Math.min(this.selectionStart, this.selectionEnd);
        int j = Math.max(this.selectionStart, this.selectionEnd);
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        if (this.isVisible()) {
            ...
            if (!string.isEmpty()) {
                ...
                n = this.textRenderer.drawShadow(matrices, this.renderTextProvider.apply(string2, this.firstCharacterIndex), (float) l, (float) m, i);
            }
            ...
            if (!string.isEmpty() && bl && j < string.length()) {
                this.textRenderer.drawShadow(matrices, this.renderTextProvider.apply(string.substring(j), this.selectionStart), (float) n, (float) m, i);
            }

            if (!bl3 && this.suggestion != null) {
                this.textRenderer.drawShadow(matrices, this.suggestion, (float) (o - 1), (float) m, -8355712);
            }
            ...
            if (bl2) {
                if (bl3) {
                    ...
                    var10002 = o + 1;
                    var10003 = m + 1;
                    fill(matrices, o, var10001, var10002, var10003 + 9, -3092272);
                } else {
                    this.textRenderer.drawShadow(matrices, "_", (float) o, (float) m, i);
                }
            }
            ...
            drawCenteredString(matrices, this.textRenderer, this.getMessage(), this.x + this.width / 2, this.y - 15, p | MathHelper.ceil(this.alpha * 255.0F) << 24);
        }
    }
    ...
    public void setSelectionEnd(int i) {
        ...
        if (this.textRenderer != null) {
            ...
            String string = this.textRenderer.plainSubstrByWidth(this.text.substring(this.firstCharacterIndex), k);
            int l = string.length() + this.firstCharacterIndex;
            if (this.selectionEnd == this.firstCharacterIndex) {
                this.firstCharacterIndex -= this.textRenderer.plainSubstrByWidth(this.text, k, true).length();
            }
