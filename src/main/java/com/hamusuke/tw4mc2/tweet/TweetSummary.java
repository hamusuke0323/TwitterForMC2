...
import com.hamusuke.tw4mc2.utils.ReplyObject;
...
public class TweetSummary implements Comparable<TweetSummary> {
	...
	public TweetSummary(Status status) {
		...
		this.photoList = ImmutableList.copyOf(this.mediaList.stream().filter(mediaEntity -> mediaEntity.getType().equals("photo")).map(TwitterPhotoMedia::new).collect(Collectors.toList()));
	...
	public void startGettingRepliesAsync(Runnable onAdd) {
		if (TwitterForMC2.getInstance().isLoggedInTwitter() && !this.isGettingReplies()) {
			this.isGettingReplies.setTrue();
			new TwitterThread(() -> {
				try {
					ReplyObject replyObject = TwitterUtil.getReplies(TwitterForMC2.getInstance().mcTwitter, this.id);
					if (replyObject != null) {
						...
						for (ReplyTweet replyTweet : replyTweets) {
							Status status = TwitterForMC2.getInstance().mcTwitter.showStatus(replyTweet.getTweetId());
	...
	public String getTweetURL() {
		return String.format("https://twitter.com/%s/status/%s", this.getScreenName(), this.getId());
	}
	...
	public String toString() {
		return "TweetSummary{" +
				"status=" + this.status +
				", retweetedStatus=" + this.retweetedStatus +
				", retweetedSummary=" + this.retweetedSummary +
				", quotedTweet=" + this.quotedTweet +
				", quotedTweetSummary=" + this.quotedTweetSummary +
				", replyCount=" + this.replyCount +
				", replyStatuses=" + this.replyStatuses +
				", replyTweetSummaries=" + this.replyTweetSummaries +
				", isGettingReplies=" + this.isGettingReplies +
				", isAlreadyGotReplies=" + this.isAlreadyGotReplies +
				", user=" + this.user +
				", userIconData=" + this.userIconData +
				", userIconFormat=" + this.userIconFormat +
				", createdAt=" + this.createdAt +
				", createdAtC=" + this.createdAtC +
				...
				", isIncludeImages=" + this.isIncludeImages +
				", isIncludeVideo=" + this.isIncludeVideo +
				", isEmptyMedia=" + this.isEmptyMedia +
				", photoMediaLength=" + this.photoMediaLength +
				", place=" + this.place +
				", retweetCount=" + this.retweetCount +
				", isRetweeted=" + this.isRetweeted +
				", retweetCountF='" + this.retweetCountF + '\'' +
				", tweet='" + this.tweet + '\'' +
				", urlList=" + this.urlList +
				", isRetweet=" + this.isRetweet +
				", isRetweetedByMe=" + this.isRetweetedByMe +
				", favoriteCountF='" + this.favoriteCountF + '\'' +
				'}';
	}
	...
	@Override
	public int compareTo(TweetSummary that) {
